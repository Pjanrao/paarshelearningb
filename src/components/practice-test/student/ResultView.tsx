    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 space-y-6 sm:space-y-8">
      ...
      <div className="bg-white rounded-[2.5rem] shadow-xl shadow-blue-900/5 border border-blue-50 overflow-hidden relative group">
        ...
        <div className="p-5 sm:p-8 md:p-12 flex flex-col items-center text-center">
          <motion.div
            ...
            className={`w-20 h-20 sm:w-28 sm:h-28 rounded-3xl flex items-center justify-center mb-6 shadow-2xl ${isPassed ? 'bg-green-100 text-green-600 shadow-green-100' : 'bg-red-100 text-red-600 shadow-red-100'
              }`}
          >
            <Award size={36} className="sm:w-[56px] sm:h-[56px]" />
          </motion.div>

          <h2 className="text-xl sm:text-3xl md:text-4xl font-black text-[#2C4276] mb-2 uppercase tracking-tight">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6 w-full max-w-2xl mb-12">
            <div className="bg-gray-50 rounded-3xl p-6 border border-gray-100 group-hover:scale-105 transition-transform duration-300">
              ...
              <div className="flex items-center justify-center gap-2">
                <span className="text-2xl sm:text-4xl font-black text-[#2C4276]">{result.score}</span>
                <span className="text-gray-300 text-2xl">/</span>
                <span className="text-base sm:text-xl font-bold text-gray-400">{result.totalMarks}</span>
              </div>
            </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 w-full justify-center">
            <Link href="/student/tests" className="flex-1 max-w-[240px]">
              <Button className="w-full sm:flex-1 sm:max-w-[240px] bg-[#2C4276] hover:bg-[#1e2e52] rounded-2xl h-14 font-black flex items-center justify-center gap-2 group-hover:shadow-2xl transition-all shadow-lg text-white">
      ...
      <div className="space-y-4">
        ...
        {result.answers.map((ans: any, i: number) => {
          ...
          return (
            <div key={i} className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden p-4 sm:p-6 md:p-8 hover:shadow-md transition-shadow">
              <div className="flex items-center gap-4 mb-6">
                <span className={`w-10 h-10 rounded-2xl flex items-center justify-center font-black text-lg ${ans.isCorrect ? 'bg-green-600 text-white' : 'bg-red-600 text-white'
                  }`}>
                  {i + 1}
                </span>
                <span className={`text-xs font-bold uppercase tracking-widest px-3 py-1 rounded-full border ${ans.isCorrect ? 'bg-green-50 text-green-600 border-green-100' : 'bg-red-50 text-red-600 border-red-100'
                  }`}>
                  {ans.isCorrect ? "Correct" : "Incorrect"}
                </span>
              </div>

              <h4 className="text-base sm:text-lg md:text-xl md:text-xl font-bold text-gray-900 mb-8 leading-relaxed">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                {question.options.map((opt: string, optIdx: number) => {
                  ...
                  return (
                    <div
                      key={optIdx}
                      className={`flex items-center gap-4 p-3 sm:p-4 rounded-2xl border-2 transition-all ${borderClass} ${bgClass}`}
                    >
                      <div className={`w-8 h-8 rounded-xl flex items-center justify-center font-bold text-xs border ${isCorrectAnswer ? "bg-green-600 text-white border-green-600" :
