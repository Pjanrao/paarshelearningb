...
//             {payment.installments?.map((i: any, index: number) => (

//               <tr key={index} className="border-t">

//                 <td className="p-2">₹{i.amount}</td>

//                 <td className="p-2">{i.paymentMode}</td>
...
export default function InstallmentHistoryModal({ payment, close }: any) {
    ...
    // Combine first payment + installments
    const history = [
        {
            amount: payment.paidAmount - (payment.installments || []).reduce((s: number, i: any) => s + Number(i.amount || 0), 0),
    ...
    ].filter(h => h.amount > 0); // safety if first payment was 0
